...
//========================================================================================================================================================
const table = document.querySelector('#pagination__table');
const pagination = document.querySelector('#pagination__list');
document.addEventListener('DOMContentLoaded', renderPreload);

async function getData() {
	const response = await fetch(
		'https://gist.githubusercontent.com/alexey1985-FE/d591d6756721ac02ea69686894b03421/raw/64614b3407860a51b3488d89f8931d1d56834de3/table.json'
	);
...
const preload = () => {
	...
	<div class="preloader">
		<div class="preloader__body">
			<img src="img/table_img/preload.svg" alt="preload">
		</div>
	</div>
	`;
...
async function mainFunc() {
	...
	let noteOnPage = 5;
	let countOfItems = Math.ceil(data.length / noteOnPage);
	...
	function showPage(item) {
		let active = document.querySelector('#pagination__list li.active');
		if (active) {
		...
		let content = '';
		notes.forEach(({ task_name, developer, work_type, status, estimation, total_time, period, efficiency }) => {
			content += `
			<tr>
			  <td data-label="Task name">${task_name}</td>	
			  <td data-label="Developer">${developer}</td>	
			  <td data-label="Work Type">${work_type}</td>	
			  <td data-label="Status">${status}</td>	
			  <td class="right padding" data-label="Estimation (h)">${estimation}</td>	
			  <td class="right padding" data-label="Total time spent">${total_time}</td>	
			  <td class="right padding" data-label="My Time spent by">${period}</td>	
			  <td class="right padding" data-label="Efficiency">${efficiency}</td>	
			</tr>`;
		});
		let tbody = `
			<thead>
				<tr>
					...
					<th>Work Type</th>
					<th>Status</th>
					<th class="right">Estimation (h)</th>
					<th class="right">Total time spent<br>
						by All </th>
					<th class="right">My Time spent by<br> Period (h)</th>
